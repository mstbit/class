...
then
	(
        	...
        	n1=$( grep -iE "sudo" /etc/group )
		IFS=',' read -r -a array <<< "$n1"
		n1=${#array[@]}
        	...
        	n2=$( grep -iE ":100[0-9]:" /etc/group | grep -ivE "sudo" | wc -l )
		...
		then
        	...
        	msg4="a01 .. a07 created and a01.tar.gz copied"
		v4=7
		if [ -e $qdir ]
		then
